// Package neeva is a lightweight hash function
/*
...
func f(msgWord uint32, cv []uint32) {
	var i, j, k int
	var word, carry, prevCarry uint32
	...
	for i = 0; i < 32; i++ {
		...
		for j = 0; j < 8; j++ {
			word = 0
			for k = 0; k < 8; k++ {
				word |= uint32(sbox[(cv[j]>>uint(28-4*k))&0xf]) << uint(28-4*k)
			}
			cv[j] = word
		}
...
func Hash(b []byte) []byte {
	...
	f(binary.BigEndian.Uint32(final[:]), cv)

	// squeezing
	hash := make([]byte, 7*4)
	binary.BigEndian.PutUint32(hash[0*4:], cv[0])
	for i := 1; i < 7; i++ {
		f(0, cv)
		binary.BigEndian.PutUint32(hash[i*4:], cv[0])
	}
